document.addEventListener('DOMContentLoaded', () => {
    ...
    window.editCar = async function(id) {
        try {
            ...
            document.getElementById('form-title').innerHTML = '<i class=\as fa-edit mr-2 text-blue-500\></i>Editar Veh�culo';
            document.getElementById('submit-btn').innerHTML = '<i class=\as fa-save mr-2\></i>Actualizar Veh�culo';

            // Scroll hacia el formulario
            window.scrollTo({ top: document.getElementById('car-form').offsetTop - 50, behavior: 'smooth' });
            
            showMsg('Puedes modificar los datos. Si no seleccionas im�genes nuevas, se mantendr�n las actuales.', 'info');
        } catch (error) {
    ...
    window.copyForSocial = async function(id) {
        try {
            ...
            text += \? Motor: \\n\n\;
            text += \?? Precio: \\n\n\;
            text += \?? �Consultanos para m�s informaci�n o permutas!\n\;
            text += \?? WhatsApp: [Tu N�mero]\n\;
            text += \?? Urquijo Automotores\n\n\;
            text += \#\ #\ #AutosUsados #UrquijoAutomotores\;

            navigator.clipboard.writeText(text).then(() => {
                alert('�Texto copiado al portapapeles! Ya pod�s pegarlo en Instagram o Facebook.');
            }).catch(err => {
